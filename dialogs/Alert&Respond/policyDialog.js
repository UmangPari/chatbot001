...
class PolicyDialog extends ComponentDialog {
    constructor(id) {
        super(id || 'policyDialog');
    ...
    async choiceStep(step)
    {
        return await step.prompt(CHOICE_PROMPT, {
            ...
            choices: ChoiceFactory.toChoices(['Show all Policies','Main Menu','BACK'])
        });
    }
    async actionStep(step)
    {
        info=step.result.value;
        if(info=='Show all Policies')
        {
            await axios.get(`${appdLink}/controller/alerting/rest/v1/applications/${appId}/policies`,
            {
                auth:
                {
                username: appdUserName,
  ...
  async finalStep(step)
  {
      ...
      {
         return await step.beginDialog('policyDialog',{app:inputApp});
      }
      else
      {   
...
module.exports.PolicyDialog = PolicyDialog;
